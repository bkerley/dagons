  ...
  module.exports = Universe = (function() {
    ...
    Universe.prototype.drawStats = function() {
      ...
      if (this.self.controls.target) {
        ...
        return this.context.fillText("targetx: " + this.self.controls.target.y, 10, 100);
      } else {
        this.context.fillText("No touch target!", 10, 90);
        return this.context.fillText("", 10, 100);
      }
    };
    ...
    Universe.prototype.enableControls = function() {
      ...
      this.board.addEventListener("mousemove", __bind(function(e) {
        if (!(this.is_drawing && !this.self.controls.anyPressed())) {
          return;
        }
        this.self.controls.target = {
          x: e.clientX,
    ...
    Universe.prototype.drawEnergyMeter = function() {
      ...
      _results = [];
      for (i = 0; i <= 10; i++) {
        this.context.fillStyle = i / 10 <= (this.self.energy / constants.maxEnergy) ? "red" : "#ccc";
    ...
    Universe.prototype.handleDeath = function() {
      if (this.self.damage > constants.deadlyDamage || this.self.dead) {
        ...
        } else if (this.self.dead >= constants.deathAnimationTime || this.self.damage >= constants.explodeyDamage) {
          this.self.damage = 0;
          this.self.dead = 0;
          this.self.speed = 0;
